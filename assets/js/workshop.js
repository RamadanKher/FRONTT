// this function  use to View All workshops
let workshops;
function ViewAllWorkshop() {
	fetch("https://enagancy.000webhostapp.com/api/workshops")
		.then(response => {
		...
		.then(data => {
			...
			for (var i = 0; i < workshops.length; i++) {
			  ...
			  <div class="service__area-item-icon mb-20 bg-white">
				  <img src="${workshops[i].small_icon}" alt="">
			  </div>
			  <div class="service__area-item-content">
				  <h4 class="mb-20 parag" style="text-align:center;">${workshops[i].small_title}</h4>
				  <p class="mb-20 cont ">${workshops[i].small_description}</p>
				  <a  onclick="getWorkshopDetils(${workshops[i].id})"    class="simple-btn"  >More Details</a>
			  </div>
		  </div>
		  </div>
				`;
			...
			document.getElementById("workshop").innerHTML = trs;
...
ViewAllWorkshop()
// this function use to get workshop Detils by id 
let singleWorkshop
 function getWorkshopDetils(id) {
	 fetch(`https://enagancy.000webhostapp.com/api/workshop/${id}`)
		.then(response => {
			return response.json()
		...
		.then(data => {
			...
			singleWorkshop = data
			console.log(data)
			localStorage.setItem("singleWorkshop", JSON.stringify(singleWorkshop));

			window.location.assign("workshop_view.html")
...
// this function use to display worksop Detils  
function displayWorkeDetils() {
	let data = JSON.parse(localStorage.getItem("singleWorkshop"));
	console.log(data)
	...
	document.getElementById("WorkDetils").innerHTML = `
	<div class="itemOne">
					<h1 class="backendTitle"> ${data.title} </h1>
					<p class="p-title">${data.description}</p>
					<div class="">
						<ul class="ListLinks">
							<li>${data.price} </li>
							<li>${data.duration} </li>
					...
					<div class="FooterButtons">
						...
						<svg xmlns="http://www.w3.org/2000/svg" width="20.402"height="24.264" viewBox="0 0 20.402 24.264">
							...
							<a href="${data.content_link}">Download Content</a>
							</button>
					</div>
	...
	`
	// change bg 
	document.getElementById("workkBg").style.cssText = `background-color:${data.color}; !important`
}
displayWorkeDetils()
...
// form workshop_view.html(Register Workshop Now)
...
function sendData() {
	...
	// (B) INIT FETCH POST
	fetch("https://enagancy.000webhostapp.com/api/workshop/regester", {
		method: "POST",
		body: data
	})
